...
contract InheritanceOracle is Ownable, ReentrancyGuard {
    ...
    event VerificationRequested(
        bytes32 indexed requestId, address indexed safe, address indexed beneficiary, VerificationType verificationType
    );

    event VerificationCompleted(bytes32 indexed requestId, VerificationStatus status, address verifier);

    event VerifierAuthorized(VerificationType verificationType, address verifier, bool authorized);
    ...
    function requestVerification(
        ...
        uint256 expiryTime = customExpiryTime > 0 ? customExpiryTime : block.timestamp + defaultExpiryPeriod;

        requestId =
            keccak256(abi.encodePacked(msg.sender, safe, beneficiary, verificationType, documentHash, block.timestamp));
     ...
     */
    function completeVerification(bytes32 requestId, bool approved, string calldata rejectionReason)
        external
        nonReentrant
    {
     ...
     */
    function generateVerificationProof(bytes32 requestId, address safe, address beneficiary)
        external
        view
     ...
     */
    function verifyProof(address safe, address beneficiary, bytes calldata proof)
        external
        view
        returns (bool isValid)
    {
        ...
        for (uint256 i = 0; i < safeRequests.length; i++) {
            ...
            if (
                request.status == VerificationStatus.VERIFIED && request.beneficiary == beneficiary
     ...
     */
    function setVerifierAuthorization(VerificationType verificationType, address verifier, bool authorized)
        external
        onlyOwner
    {
     ...
     */
    function updateSettings(uint256 _defaultExpiryPeriod, uint256 _verificationFee) external onlyOwner {
     ...
     */
    function getSafeVerifications(address safe) external view returns (bytes32[] memory requestIds) {
     ...
     */
    function isAuthorizedVerifier(VerificationType verificationType, address verifier)
        external
        view
