...
contract InheritanceSecurity is AccessControl, Pausable, ReentrancyGuard {
    ...
    function getSuspiciousActivities(address safe) 
        external 
        view 
        returns (SuspiciousActivity[] memory activities) 
    {
    ...
    function getSecurityConfig(address safe) 
        external 
        view 
        returns (SecurityConfig memory config) 
    {
        return safeSecurityConfigs[safe];
    }

    function getRateLimit(address safe) 
        external 
        view 
        returns (RateLimit memory limit) 
    {
